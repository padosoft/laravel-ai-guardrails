...
| GET | `/audit/trend` | `…audit.trend` | `…v1.audit-trend` | per-UTC-day SQL `GROUP BY` (dialect-safe); 30-day default window; **v1.1.0 adds** `points[].observed` bucket — an additive **subset of `allowed`** (`observed ⊆ allowed`); the v1.0 invariant `total === blocked + allowed` is preserved (disjoint "no rule matched" series = `allowed − observed`) |
| GET | `/firewall` | `…firewall.index` | `…v1.firewall` | `firewall_log.store` — Control A rejections, keyset paginated |
| GET | `/provenance` | `…provenance.index` | `…v1.provenance` | `provenance_log.store` — Control P decisions, keyset paginated. `?blocked=0` is the **monitor-rollout view**: calls that RAN and would have been refused under enforcement. Carries no tool arguments by design |
