...
- **Untrusted query params.** Read every scalar param as string-or-null (`is_string && !== ''`); a repeated param (`limit[]=10`) makes `query()` return an array and must never reach `filter_var`/casts (would 500 a read-only endpoint). `blocked` uses `FILTER_NULL_ON_FAILURE` so garbage = "no filter", not `false`.
- **Untrusted stored text in resources.** Audit prompts / firewall tool+violation text may be invalid UTF-8 (the audit logs every attempt). Scrub with `mb_check_encoding` + `mb_scrub` before any `mb_*` and before returning, else `json_encode` fails. `matched_span` (byte offset) is only emitted when the returned bytes equal the stored bytes (normalization/scrubbing can shift offsets).
- **Settings runtime overlay.** `settings.store=database` only matters if the saved overrides actually reach the controls: the provider overlays them onto `config('ai-guardrails.*')` at the START of `register()` (delegating to `DatabaseGuardrailSettingsStore::all()` so the decode + fail-safe policy lives in one place). Fail-safe: skip corrupt JSON, `null`, and type-mismatched rows (`(bool) null` would silently disable a control). Effective next boot.
- **laravel-flow integration test.** `loadMigrationsFrom` is NOT wired in this package's Testbench harness — run the real flow migrations manually in `setUp` (glob the vendor dir, `sort()` for timestamp order, `(require $f)->up()`), boot `LaravelFlowServiceProvider`, set `laravel-flow.persistence.enabled=true` + a file cache. The approval row's `payload` does NOT carry the flow input (tool name lives on `flow_runs.input`), so scope `flow_approvals` reads to `FlowApprovalRouter::FLOW_NAME` via a `whereIn` subquery on `flow_runs.definition_name`, and register the flow definition before `Flow::resume/reject` (a fresh worker never called `route()`).
- **PSR-3 logging.** Pass the `Throwable` (not `$e->getMessage()`) under the reserved `exception` context key so Monolog records the type + stack.

### MCP surface L5 — security review fixes (2026-06-18)
- **`ruleset_version` fingerprinting (MEDIUM).** `ScreenPromptTool` originally returned `ruleset_version` on every call, even for benign prompts. An adversarial MCP client (or jailbroken model calling the tool) can probe the exact ruleset version without triggering a block, then target version-specific bypass techniques. Fix: gate `'ruleset_version' => $verdict->blocked ? $verdict->rulesetVersion : null` so the version is only disclosed when a rule actually matched.
- **`SanitizeOutputTool` unbounded input (MEDIUM).** No `maxLength` guard meant a client could submit arbitrarily large text and drive HTMLPurifier / PII redactor into CPU/memory saturation. Fix: `MAX_TEXT_LENGTH = 65_535` constant, `->max(self::MAX_TEXT_LENGTH)` in JSON Schema (client-visible), and a `mb_strlen` guard in `handle()` returning `Response::error()` when exceeded. Test: `assertHasErrors(['65535'])`.
- **`RecentAuditTool` principal-id exposure (LOW).** `principal_id` (an internal user/session identifier) was always included in the response, letting the MCP client (and any AI model invoking the tool) correlate user identities with injection-block history. Fix: omit by default; expose only when caller passes `include_principal_ids=true`. Added boolean parameter to schema with a warning description. Tests: `assertDontSee('principal_id')` by default, `assertSee('principal_id')` + `assertSee('user-42')` when opt-in.
- **MCP `TestResponse` assertion API.** `assertSee('isError')` does NOT work for error responses — `assertSee` checks `$this->content()` (the text blobs), not field names in the JSON envelope. Use `assertHasErrors(['substring'])` instead; it checks `result.isError=true` then content text, or the top-level `error.message`.

- **GDPR actor-audit ordering (MEDIUM):** The purge command originally wrote `Log::info` AFTER the DB mutation. If the log channel is down, erasure happens without an audit trail — the explicit accountability guarantee is broken. Fix: write a pre-audit `Log::info('…starting', [actor,strategy,cutoff])` BEFORE `delete()`/`update()`, then a post-audit entry with the affected-row count. The pre-audit is the GDPR-accountable record; the post-audit provides the row count.
