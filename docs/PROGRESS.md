...
- **L2 — HTML purify (Control C) — DONE locally, branch `feature/v1-l2-html-purify`.** `PurifierHtmlSanitizer` (composes HTMLPurifier for HTML + delegates markdown defang) + `HtmlSanitizerFactory` (html_mode=allowlist → HTMLPurifier when installed, graceful fallback to built-in allowlist; vendor ref confined to src/Output, arch test extended). `ezyang/htmlpurifier` ^4.19 require-dev + suggest. Tests: factory both-states + purifier (script/onerror/attr/malformed/markdown defang/report). **444 tests** GREEN; pint+phpstan+validate clean. [ ] DoD loop → PR.
- **L3 — toolCalls opt-in defense-in-depth (Control C) — DONE locally, branch `feature/v1-l3-toolcalls`.** Config `output_handler.sanitize_tool_calls` (default **false** → tools never altered). When on, GuardrailOutputMiddleware cleans the string leaves of each tool call's arguments (handles both production `ToolCall` objects and plain-array shapes); monitor records stats but doesn't mutate. Tests: default-off untouched, opt-in array + ToolCall shapes, nested leaves, monitor-no-mutate. README limitation + config updated. **449 tests** GREEN; pint+phpstan clean. [ ] DoD loop → PR.
- **L4 — HITL turnkey (Control D) — DONE locally, branch `feature/v1-l4-hitl-turnkey`.** `ai-guardrails:hitl-status` (diagnostic: flow installed / persistence / flow tables / hitl+master toggles; non-zero exit + guidance when misconfigured) + `ai-guardrails:hitl-install` (runs laravel-flow migrations scoped via `migrate --path` straight from vendor — no publish/pollution, idempotent). Both in src/Hitl (compose-not-couple; arch test green). Tests: status ready/tables-missing/hitl-off + install creates tables. README HITL-setup section. **454 tests** GREEN; pint+phpstan clean. [ ] DoD loop → PR.
- **L5 — MCP surface (4th surface) — DONE locally, branch `feature/v1-l5-mcp`.** `GuardrailsMcpServer` + tools `ScreenPromptTool`/`SanitizeOutputTool`/`RecentAuditTool` (extend `Laravel\Mcp\Server\Tool`); `McpServerRegistrar::registerIfAvailable()` registers via `Mcp::local('ai-guardrails', …)` in boot when `mcp.enabled` (default OFF) + laravel/mcp present. `laravel/mcp` ^0.8 require-dev + suggest; vendor ref confined to src/Mcp (arch test extended); src/Mcp excluded from mutation (IO adapter, like Http/Console). Tests: each tool via laravel/mcp's server test harness (registering its provider so the resolving(Request) hook injects args) + registration both-states. README MCP section. Security hardening (post-review): `ruleset_version` only returned when blocked (no adversarial fingerprinting on benign calls); `SanitizeOutputTool` enforces 65 535-char max with schema + handle guard; `RecentAuditTool` omits `principal_id` by default (opt-in via `include_principal_ids=true`). **464 tests** GREEN; pint+phpstan clean. [ ] DoD loop → PR → then tag **v1.0.0**.

### ✅ v1.0.0 RELEASED (2026-06-18) — all documented limitations closed
- L1 confusables (PR #28) · L2 HTMLPurifier (PR #29) · L3 toolCalls opt-in (PR #30) · L4 HITL turnkey (PR #31) · L5 MCP surface (PR #32) — all merged to `main`, main CI green at closure SHA `11bd0c1`.
- Tagged **v1.0.0** + GitHub Release "Every documented limitation, closed". https://github.com/padosoft/laravel-ai-guardrails/releases/tag/v1.0.0
- **464 tests / 1182 assertions** green (PHP 8.3/8.4/8.5 × Laravel 13); PHPStan level 8; Pint clean; Infection ≥80% MSI in CI.
- Compose-not-couple extended: flow→src/Hitl, pii+HTMLPurifier→src/Output, laravel/mcp→src/Mcp (arch test). Every optional dep is `suggest` with graceful degradation.
- **Nothing outstanding.** README "Known limitations" now describes residual design boundaries only (curated-not-exhaustive confusables map; toolCalls opt-in by design).
